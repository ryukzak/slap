...
3. Student registers for the lesson before it starts — registration is blocked once the lesson begins.
   ...
   - On the task page, a registered (queued) student sees their position in the queue ("#N of M in queue"), based on submission time and excluding revoked records. On the lesson page, the student sees their position next to the registered tasks header for each of their queued tasks, following the currently selected sort order.
